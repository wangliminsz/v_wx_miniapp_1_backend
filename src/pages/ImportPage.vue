<template>
  <div class="import-page">
    ...
    <div class="mt-6 bg-dark-200 p-6 rounded-md border border-dark-100">
      ...
      <!-- Import Facets Section -->
      <div class="mt-6 bg-dark-200 p-6 rounded-md border border-dark-100">
        <h2 class="text-xl font-bold text-blue-300 mb-4">Import Facets & Facet Values</h2>
        
        <div class="mb-4">
          <p class="text-gray-300">Import facet groups and their values from a CSV file.</p>
          <p class="mt-2 text-sm text-gray-400">
            CSV Format:
            <br>
            <code class="bg-dark-300 px-2 py-1 rounded">facetCode,facetName,facetValueCode,facetValueName</code>
          </p>
          <p class="mt-2 text-xs text-gray-400">
            Example:<br>
            <code class="bg-dark-300 px-2 py-1 rounded">category,Category,electronics,Electronics</code><br>
            <code class="bg-dark-300 px-2 py-1 rounded">category,Category,computers,Computers</code><br>
            <code class="bg-dark-300 px-2 py-1 rounded">brand,Brand,apple,Apple</code>
          </p>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-1 gap-6">
          ...
          <div>
            ...
            <input 
              type="file" 
              accept=".csv" 
              @change="handleFileChange('facets', $event)" 
            ...
            >
            <div v-if="importFiles.facets" class="mt-2 text-xs text-green-400">
              Selected: {{ importFiles.facets.name }}
            </div>
          </div>
        </div>
        ...
        <div class="mt-6 flex gap-4">
          <button 
            @click="importFacets"
            :disabled="!importFiles.facets || isImporting"
            class="px-6 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-500 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
          >
            {{ isImporting ? 'Importing...' : 'Import Facets & Values' }}
          </button>
          
          <button 
            @click="clearFile('facets')"
            :disabled="!importFiles.facets"
    ...
    <!-- Import Collections Section -->
    <div class="mt-6 bg-dark-200 p-6 rounded-md border border-dark-100">
      <h2 class="text-xl font-bold text-blue-300 mb-4">Import Collections</h2>
      
      <div class="mb-4">
        <p class="text-gray-300">Import or update assets for collections. If a collection doesn't exist, it will be created automatically.</p>
      </div>
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div>
          ...
          <input 
            type="file" 
            accept=".csv" 
            @change="handleFileChange('collectionAssetsCsv', $event)" 
            class="w-full px-4 py-2 bg-dark-300 text-white rounded-md border border-dark-100 focus:outline-none focus:border-secondary focus:ring-2 focus:ring-secondary/50 transition-colors"
          >
          <p class="mt-1 text-xs text-gray-400">
            Required columns: collectionSlug, collectionFeaturedAsset, collectionAssets<br>
            Optional columns: collectionName, parentSlug, facetValueCodeForFilter
          </p>
          <p class="mt-1 text-xs text-gray-400">
            Example:<br>
            <code class="bg-dark-300 px-2 py-1 rounded">new-collection,New Collection,,featured.jpg,asset1.jpg|asset2.jpg,new-facet</code><br>
            <code class="bg-dark-300 px-2 py-1 rounded">hepatitis_c,丙肝,liver_medications,丙肝抽象图-.jpg,,hepatitis_c</code>
          </p>
          <div v-if="importFiles.collectionAssetsCsv" class="mt-2 text-xs text-green-400">
            Selected: {{ importFiles.collectionAssetsCsv.name }}
          </div>
        </div>
      ...
      <div class="mt-6">
        <h3 class="text-sm font-medium text-gray-300 mb-2">Collection Slug Helper:</h3>
        ...
        <div v-if="selectedCollection" class="mt-2 bg-dark-300 p-3 rounded-md">
          <p class="text-sm">
            <span class="text-gray-400">Slug:</span> 
            <code class="text-blue-300 bg-dark-400 px-2 py-1 rounded">{{ selectedCollection.slug }}</code>
          </p>
        </div>
        <p class="mt-1 text-xs text-gray-400">Use this slug in your CSV file for the collectionSlug column.</p>
      </div>
      
      <!-- Import Button -->
      <div class="mt-6 flex gap-4">
        <button 
          @click="importCollectionAssets"
          :disabled="!importFiles.collectionAssetsCsv || !importFiles.assetsMapJson || isImportingCollectionAssets"
          class="px-6 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-500 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
        >
          {{ isImportingCollectionAssets ? 'Importing...' : 'Import Collection Assets' }}
        </button>
        
        <button 
          @click="clearCollectionAssetsFiles"
          :disabled="!importFiles.collectionAssetsCsv && !importFiles.assetsMapJson"
          class="px-6 py-2 bg-gray-600 text-white rounded-md hover:bg-gray-500 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
        >
          Clear Files
        </button>
      ...
      <!-- Import Collections Section -->
      <div class="mt-6 bg-dark-200 p-6 rounded-md border border-dark-100">
        <h2 class="text-xl font-bold text-blue-300 mb-4">Import Collections - Filter</h2>
        
        <div class="mb-4">
          <p class="text-gray-300">Import collections from a CSV file with parent-child relationships support and facet value filters.</p>
          <p class="mt-2 text-sm text-gray-400">
            CSV Format:
            <br>
            <code class="bg-dark-300 px-2 py-1 rounded">collectionSlug,collectionName,parentSlug,collectionFeaturedAsset,collectionAssets,facetValueCodeForFilter</code>
          </p>
          <p class="mt-2 text-xs text-gray-400">
            Example:<br>
            <code class="bg-dark-300 px-2 py-1 rounded">root-collection,Root Collection,,image1.jpg,image2.jpg|image3.jpg,</code><br>
            <code class="bg-dark-300 px-2 py-1 rounded">child-collection,Child Collection,root-collection,child1.jpg,child2.jpg,child-facet</code><br>
            <code class="bg-dark-300 px-2 py-1 rounded">hepatitis_c,丙肝,liver_medications,丙肝抽象图-.jpg,,hepatitis_c</code>
          </p>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div>
            ...
            <input 
              type="file" 
              accept=".csv" 
              @change="handleFileChange('collections', $event)" 
              class="w-full px-4 py-2 bg-dark-300 text-white rounded-md border border-dark-100 focus:outline-none focus:border-secondary focus:ring-2 focus:ring-secondary/50 transition-colors"
            >
            <div v-if="importFiles.collections" class="mt-2 text-xs text-green-400">
              Selected: {{ importFiles.collections.name }}
            </div>
          </div>
          
          <!-- Assets Map JSON File Upload -->
          <div>
            <label class="block text-sm font-medium text-gray-300 mb-2">Assets Map JSON File (Optional):</label>
            <input 
              type="file" 
              accept=".json" 
              @change="handleFileChange('assetsMapJson', $event)" 
        ...
        <div class="mt-6 flex gap-4">
          <button 
            ...
            :disabled="!importFiles.collections || isImporting"
            class="px-6 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-500 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
          >
          ...
          <button 
            @click="clearFile('collections')"
            :disabled="!importFiles.collections"
            class="px-6 py-2 bg-gray-600 text-white rounded-md hover:bg-gray-500 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
          >
            Clear
          </button>
...
const fetchCollections = async () => {
  ...
  try {
    const apolloClient = createApolloClient(authStore.token, getChannelTokenFromQuery() || authStore.activeChannel?.token || null)
