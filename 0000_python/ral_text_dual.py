# python ral_dual_text.py --ral 5018 --text1 "重防腐" --text2 "粉末"


"""Render a RAL swatch with two lines of Chinese text.

Usage:
  python ral_dual_text.py --ral 5018 --text1 "重防腐" --text2 "粉末"
  python ral_dual_text.py --ral 7040 --text1 "户外耐候" --text2 "粉末" -o out.jpg
"""
...
def render_color(color, lines, out_path,
    ...
    max_w = max(text_size(draw, s, font_main)[0] for s in lines)
    ...
    if len(lines) == 1:
        draw_centered_text_with_mask(
            img_rgba, draw, lines[0], font_main,
...
def main(argv=None):
    p = argparse.ArgumentParser(
        description="Render RAL swatch with two lines of Chinese text.",
    )
    p.add_argument("--ral", metavar="CODE", required=True,
                   help="RAL color number (e.g. 5018).")
    p.add_argument("--text1", metavar="TEXT", required=True,
                   help="First line of text (top).")
    p.add_argument("--text2", metavar="TEXT", required=True,
                   help="Second line of text (bottom).")
    ...
    lines = [args.text1, args.text2]

    if args.output:
        out_path = Path(args.output)
    else:
        OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
        label = f"{safe_filename_part(args.text1)}_{safe_filename_part(args.text2)}"
        out_path = OUTPUT_DIR / f"DUAL_RAL{args.ral}_{label}.jpg"
